    ...
    xml.Table do
      ...
      xml.Row 'ss:StyleID'=>"Heading" do
        @transaction_fields.each do | field |
          if ( field == 'time' )
            xml.Cell do
              xml.Data "#{l :short_date}", 'ss:Type'=>"String"
            end
            xml.Cell do
              xml.Data "#{l :time}", 'ss:Type'=>"String"
            end
          else
            xml.Cell do
              xml.Data "#{l field}", 'ss:Type'=>"String"
            end
          end
        end
      end
          ...
          @transaction_fields.each do | field |
            ...
            if ( field == 'time' )
              ...
              xml.Cell field_map[:excel_style] ? {'ss:StyleID'=> field_map[:excel_style]} : {} do
                xml.Data value, 'ss:Type'=> field_map[:excel_type] ? field_map[:excel_type] : "String"
              end
            end
          end
        end
